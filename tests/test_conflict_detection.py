...
This script tests the should_update_field and merge_parcel_into_enrichment
functions without making actual API calls.
"""
...
from scripts.extract_county_data import merge_parcel_into_enrichment, should_update_field
...
def test_should_update_field():
    """Test the should_update_field function with various scenarios."""
    print("\n" + "=" * 60)
    ...
    should_update, reason = should_update_field(entry, "lot_sqft", 8500, logger)
    ...
    should_update, reason = should_update_field(entry, "sewer_type", "septic", logger)
    ...
    print("\n3. No existing value:")
    entry = {}
    should_update, reason = should_update_field(entry, "lot_sqft", 8000, logger)
    ...
    should_update, reason = should_update_field(entry, "year_built", 1980, logger)
    ...
    should_update, reason = should_update_field(entry, "garage_spaces", 2, logger)
    ...
    should_update, reason = should_update_field(entry, "has_pool", True, logger)
...
def test_merge_parcel_into_enrichment():
    """Test the merge_parcel_into_enrichment function."""
    print("\n" + "=" * 60)
    print("Testing merge_parcel_into_enrichment()")
    print("=" * 60)
    ...
    existing = {}
    entry, conflicts = merge_parcel_into_enrichment(
        existing, parcel.full_address, parcel, update_only=False, logger=logger
    )
    ...
    entry, conflicts = merge_parcel_into_enrichment(
        existing, parcel.full_address, parcel, update_only=True, logger=logger
    )
    ...
    print("\n✓ All merge_parcel_into_enrichment tests passed!")
